...
function getBrowserType() {
  ...
  // Determine whether it is an Edge browser
  ...
  // Determine whether it is the Opera browser
  if (userAgent.includes('Opera')) {
    return userAgentKey.Opera;
  }
  if (/iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream) {
    return userAgentKey.Ios;
  }
  return '';
}
...
function getBrowserVersion(browserType) {
  ...
  return browserType == 'Ios'
    ? ''
    : userAgent.split(browserType)[1].split('.')[0].slice(1);
}
...
function CommonAudioUtil() {
  ...
  this.predictorPole = function (statePtr) {
    return (
      fmult(statePtr.pp[1] >> 2, statePtr.sr[1]) +
      fmult(statePtr.pp[0] >> 2, statePtr.sr[0])
    );
  };
  ...
  this.update = function (
    ...
    if (tr === 1) {
      ...
      Math.abs((statePtr.dms << 2) - statePtr.dml) >=
      statePtr.dml >> 3
    ) {
      statePtr.ppp += (0x200 - statePtr.ppp) >> 4;
    } else {
